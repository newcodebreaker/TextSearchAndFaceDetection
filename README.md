...
In this project, we read a zipped folder which contains some image files. We search for a given text in the image files and if the text is present in that image we detect all the faces in that image, crop them and paste them on a canvas for display.   
